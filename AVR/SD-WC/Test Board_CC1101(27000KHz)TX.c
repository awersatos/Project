...
{ PORTA.2=1; //Светодиод 1 
  PORTA.7=1; //Светодиод 2 
  delay_ms(1000);
  if((PINB.0==0)&&(PINB.1==0))SEND_PAKET(Write_SPI_buffer("DATCHIK128128")); 
  

 } 
 else{
if((PINB.0==0))  SEND_PAKET(Write_SPI_buffer("BRELOK1-READ")); 
if((PINB.1==0))  SEND_PAKET(Write_SPI_buffer("BRELOK1-CHANGE")); } 
 CLEAR_SPI_buffer();
  GIFR|=0x10;
  GIMSK=0x10;
  STROB(SRX);
