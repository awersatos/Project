...
//**********************************************************************************************************
       void SEND_Str(flash char *str) {        // Функция передачи строки
        while(*str) { 
       UART_Transmit(*str++);
...
m5:       SEND_Str("AT+CMGS=\"+7");  // Ввод номера SMS
       for (i=0; i<10; i++)
      {UART_Transmit(NR[i]);}
      SEND_Str("\",145\r"); 
      ...
      SEND_Str("DOOR OPEN\x1A");     // Ввод текста SMS
   ...
   //   delay_ms(1000);
     ...
     // delay_ms(10000);
      ...
      delay_ms(2000);
       #asm("cli")
      for (i=0; i<56; i++)    // Запись буфера приема в eeprom
