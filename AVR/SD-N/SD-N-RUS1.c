...
{z=128;     
      ...
      while(PIND.5==1); 
                            /*
      //while(TEST_OK()==0) { SEND_Str("AT\r");}  // Проверка ответа модема
       // SEND_Str("AT+CREG?\r");
     while(REG_NET()!='1') {           // Проверка регистрации в сети
      
       SEND_Str("AT+CREG?\r");
      //delay_ms(1000); 
      }         
                      */
   while(TEST_OK()==0) {   SEND_Str("AT+CMGF=0\r"); }    // Установка PDU режима 
     //  if (TEST_OK()==0) goto mx3 ;
                    
      SEND_Str("AT+CMGS=43\r");  //    Ввод команды отправки сообщения
   ...
   /*   if (strrchr(rx_buffer, '>')==NULL)
      ...
      {UART_Transmit(NR[i]);}
       ...
       SEND_Str("00181E04120437044F0442043E0020043D04300020043E044504400430043D0443\x1A");
    //  if (TEST_ERROR()=='E') goto mx4;         // Ввод текста SMS
       PORTD.4=1; // RTS
       PORTD.6=1;  // DTR    */  
}
else {z=0;}
...
m5:       SEND_Str("AT+CMGS=39\r");  //    Ввод команды отправки сообщения
      ...
      if (TEST_ERROR()=='E') goto m5;         // Ввод текста SMS
       ...
       SEND_Str("AT+CSQ?\r");
        delay_ms(1000);
       #asm("cli")
      for (i=0; i<50; i++)    // Запись буфера приема в eeprom
